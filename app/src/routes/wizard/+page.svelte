<script lang="ts">
  ...
  import { defaultStepState, type WizardStepState } from "$lib/components/wizard/step-state";
...
<div class="h-full flex flex-col">
  ...
  <footer class="px-8 py-4 border-t border-border flex items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-4">
      ...
      {/if}
      <Button onclick={onFooterNext} disabled={!activeState.canNext || nextPending}>
...
<AlertDialog.Root bind:open={skipPromptOpen}>
  <AlertDialog.Content>
    ...
    <AlertDialog.Footer>
      ...
      <AlertDialog.Cancel variant="default"
        >{t("wizard.common.skip_confirm_cancel")}</AlertDialog.Cancel
      >
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
