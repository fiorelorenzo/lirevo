...
const OTHER_LABEL: Record<Modifier, string> = { control: 'Ctrl', option: 'Alt', command: 'Super', shift: 'Shift' };

function modChips(m: ModifierFlags, os: Os): string[] {
  const order: Modifier[] = ['control', 'option', 'command', 'shift'];
  const chips = order.filter((k) => m[k]).map((k) => (os === 'macos' ? MAC_GLYPH[k] : OTHER_LABEL[k]));
...
const MAC_RESERVED = new Set([
  'command+Q', 'command+W', 'command+Space', 'command+Tab', 'command+H', 'command+M',
