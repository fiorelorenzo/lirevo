...
describe("validateHotkey", () => {
  ...
  it("accepts a combo", () => {
    expect(
      validateHotkey({ modifiers: { command: true, shift: true }, trigger: { key: "K" } }, mac).ok,
    ).toBe(true);
  });
  it("rejects > 3 keys", () => {
    const spec: HotkeySpec = {
      modifiers: { control: true, command: true, shift: true },
      trigger: { key: "K" },
    };
    expect(validateHotkey(spec, mac).ok).toBe(false);
  });
  it("rejects a critical system shortcut (Cmd+Q)", () => {
    expect(validateHotkey({ modifiers: { command: true }, trigger: { key: "Q" } }, mac).ok).toBe(
      false,
    );
  });
