    ...
    if (e.modOnly) {
      ...
      if (!modOnlyTimer) {
        modOnlyTimer = setTimeout(() => {
          modOnlyTimer = null;
          if (pendingModOnly) finalize({ modifiers: {}, trigger: { modifierOnly: pendingModOnly } });
        }, 200);
      }
...
<div class="space-y-3">
  <div class="flex items-center gap-3">
    ...
    >
      {#if capturing}
        <span class="hotkey-pulse text-sm text-muted-foreground">Listening… press your shortcut</span>
