...
async fn cmd_clean(
    ...
    let input: String = match text.as_deref() {
        Some("-") | None => {
            ...
            if buf.trim().is_empty() {
                eprintln!(
                    "error: no input. Pass text as argument, or pipe via stdin: `lirevo-cli clean -` or `... | lirevo-cli clean`"
                );
                return Ok(5);
            }
            buf
    ...
    let system = lirevo_prompts::build_clean_system_prompt(&language);
...
async fn main() -> std::process::ExitCode {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("LIREVO_CLI_LOG_LEVEL").unwrap_or_else(|_| "warn".to_string()),
        )
