# lda-eval — Refiner model evaluation harness
...
cargo run -p lda-eval --release -- run \
  ...
  --out crates/lda-eval/data/reports/$(date +%Y-%m-%d)-bake-off.md

# 2. (Optional) Re-score with Claude as judge.
cargo run -p lda-eval --release -- judge \
  --report crates/lda-eval/data/reports/$(date +%Y-%m-%d)-bake-off.json \
  --judge claude-p:claude-3-5-sonnet \
  --out crates/lda-eval/data/reports/$(date +%Y-%m-%d)-judged.md

# 3. (Optional) Grow the corpus with an oracle.
cargo run -p lda-eval --release -- gen-corpus \
  --oracle claude-p:claude-3-5-sonnet \
  --seeds crates/lda-eval/data/corpus/v1-seed.jsonl \
  --profiles crates/lda-eval/data/profiles/v1.toml \
  --target-per-cell 4 \
  --out crates/lda-eval/data/corpus/v1.jsonl
...
# downloaded into crates/lda-eval/.cache/ (gitignored) and its SHA-256 is
...
Validation: `cargo test -p lda-eval --test data_validates`.
...
Run `cargo test -p lda-eval --test data_validates` to confirm the new
...
- `<out>.json` — machine-readable sidecar, consumable by `lda-eval judge`.
...
- `ort` v2 with default features fetches an ONNX Runtime native lib at
  build time. First clean build of `lda-eval` needs network access.
